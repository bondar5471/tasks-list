...
const FormAddTask = ({onNewTask = (f) => f}) => {
  let list, date_end, duration
    ...
    const datePic = e => {
      e.preventDefault()
      let durationTask = duration.value
      if(durationTask === "year") {
        let taskYearDate = moment().format('YYYY-12-31');
        document.getElementById('date').value = taskYearDate
      }
      if(durationTask === "month") {
        let taskMonthDate = moment().endOf('month').format('YYYY-MM-DD');
        document.getElementById('date').value = taskMonthDate
      }
      if(durationTask === "day") {
        let taskDayDate = moment().format('YYYY-MM-DD');
        document.getElementById('date').value = taskDayDate
      }
      if(durationTask === "week") {
        let taskWeekDate = moment().endOf('week').add(1, 'days').format('YYYY-MM-DD');//calendar location
